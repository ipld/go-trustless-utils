...
func TestUnixfs20mVariety(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.Name, func(t *testing.T) {
			...
			carStream, errorCh := makeCarStream(t, ctx, []cid.Cid{tc.Root}, expectedBlocks, false, false, false, nil, false, false)
			...
			// the same query.
			scope, err := trustlessutils.ParseDagScope(tc.Scope)
			req.NoError(err)
			var byteRange *trustlessutils.ByteRange
			if tc.ByteRange != "" {
				br, err := trustlessutils.ParseByteRange(tc.ByteRange)
				req.NoError(err)
				byteRange = &br
			}
			cfg := traversal.Config{
				Root:     tc.Root,
				Selector: trustlessutils.Request{Path: tc.Path, Scope: scope, Bytes: byteRange}.Selector(),
			}
			{
			...
			req.NoError(err)
			req.Equal(count(expectedBlocks), result.BlocksIn)
			req.Equal(sizeOf(expectedBlocks), result.BytesIn)
			req.Equal(count(expectedBlocks), result.BlocksOut)
			req.Equal(sizeOf(expectedBlocks), result.BytesOut)
...
func TestVerifyCar(t *testing.T) {
	...
	testCases := []struct {
		...
		carAsCIDv0         bool
		carAsRawBlocks     bool
		checkPathWith      datamodel.Path
		expectCheckPathErr string
		expectBlocksIn     uint64
		expectBytesIn      uint64
		expectBlocksOut    uint64
		expectBytesOut     uint64
	}{
		{
			name:   "complete carv1",
		...
		},
		{
			...
			blocks: consumedBlocks(allBlocks),
			roots:  []cid.Cid{root1},
			expectErr: (&ipldtraversal.ErrBudgetExceeded{
				BudgetKind: "link",
		...
		},
		{
			...
			checkPathWith: datamodel.ParsePath(wrapPath),
		},
		{
		...
		},
		{
			...
			incomingHasDups: true,
			expectBlocksIn:  uint64(len(unixfsFileWithDupsBlocks)),
			expectBytesIn:   sizeOf(consumedBlocks(unixfsFileWithDupsBlocks)),
			expectBlocksOut: 3,
			expectBytesOut:  sizeOf(append(append(consumedBlocks(unixfsFileWithDupsBlocks[:2]), skippedBlocks(unixfsFileWithDupsBlocks[2:len(unixfsFileWithDupsBlocks)-1])...), consumedBlocks(unixfsFileWithDupsBlocks[len(unixfsFileWithDupsBlocks)-1:])...)),
		},
		{
			...
			expectBlocksIn:  3,
			expectBytesIn:   sizeOf(append(append(consumedBlocks(unixfsFileWithDupsBlocks[:2]), skippedBlocks(unixfsFileWithDupsBlocks[2:len(unixfsFileWithDupsBlocks)-1])...), consumedBlocks(unixfsFileWithDupsBlocks[len(unixfsFileWithDupsBlocks)-1:])...)),
			expectBlocksOut: uint64(len(unixfsFileWithDupsBlocks)),
			expectBytesOut:  sizeOf(consumedBlocks(unixfsFileWithDupsBlocks)),
		},
		{
		...
		{
			name:   "unixfs: small single-block file wrapped in directories, with CheckPath",
	...
	for _, testCase := range testCases {
		...
		t.Run(testCase.name, func(t *testing.T) {
			...
			if testCase.expectErr == "" {
				req.NoError(err)
				if testCase.expectBlocksIn > 0 {
					req.Equal(testCase.expectBlocksIn, result.BlocksIn)
				} else {
				...
				if testCase.expectBlocksOut > 0 {
					...
					req.Equal(count(testCase.blocks), result.BlocksOut)
				}
				if testCase.expectBytesIn > 0 {
					req.Equal(testCase.expectBytesIn, result.BytesIn)
				} else {
				...
				if testCase.expectBytesOut > 0 {
					...
					req.Equal(sizeOf(testCase.blocks), result.BytesOut)
				}
				req.Equal(int(count(testCase.blocks)), writeCounter)
			} else {
				req.ErrorContains(err, testCase.expectErr)
				req.Equal(uint64(0), result.BlocksIn)
				req.Equal(uint64(0), result.BytesIn)
				req.Equal(uint64(0), result.BlocksOut)
				req.Equal(uint64(0), result.BytesOut)
			}

			if testCase.checkPathWith.Len() > 0 {
				err = traversal.CheckPath(testCase.checkPathWith, result.LastPath)
				if testCase.expectCheckPathErr != "" {
					req.ErrorContains(err, testCase.expectCheckPathErr)
				} else {
					req.NoError(err)
				}
			}
